...
const TeamPreview = () => {
  ...
  const categories = [
    { id: 'core', name: 'Core Team', members: coreTeam, icon: '👑', color: 'from-ancient-gold to-saffron' },
    { id: 'technical', name: 'Technical Team', members: technicalTeam, icon: '💻', color: 'from-saffron to-burnt-orange' },
    { id: 'management', name: 'Management', members: managementTeam, icon: '📊', color: 'from-burnt-orange to-sandstone' },
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
          {categories.map((category, index) => (
            <motion.button
              ...
              layout
              className={`bg-charcoal/50 backdrop-blur-sm border-2 rounded-xl p-6 sm:p-8 text-center transition-hover bg-gradient-to-br ${category.color} ${
                selectedCategory === category.id
                  ? 'border-ancient-gold glow-gold opacity-100'
                  : 'border-ancient-gold/30 hover:border-ancient-gold/60 opacity-80 hover:opacity-100'
              }`}
            >
              ...
              <p className="text-sm sm:text-base text-gray-200">{category.members.length} Members</p>
            </motion.button>
          ))}
        </div>
